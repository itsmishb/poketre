...
# sheets（既定）| postgres（ocr_staging へ直接 INSERT。DATABASE_URL 必須）
OCR_WRITE_TARGET = os.environ.get("OCR_WRITE_TARGET", "sheets").strip().lower()
DATABASE_URL_PG = os.environ.get("DATABASE_URL")
...
CONCURRENT_WORKERS = 5 # Reduced from 10 for stability (prevent SSL/Segfault)
CONCURRENT_DRIVE_WORKERS = 2 # Reduced from 3

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

app = Flask(__name__)
WORKER_SHARED_SECRET = os.environ.get("OCR_WORKER_SHARED_SECRET", "").strip()

def mask_api_key(url):
    return re.sub(r'key=[^&\s]+', 'key=***MASKED***', url)

def safe_api_call(max_retries=3, backoff_in_seconds=2):
        ...
        def wrapper(*args, **kwargs):
            ...
            while t < max_retries:
                try:
                    ...
                    if t == max_retries - 1:
                        raise
                    sleep_time = (backoff_in_seconds ** t)
                    logger.warning(f"API call failed: {e}. Retrying in {sleep_time}s... (Attempt {t+1}/{max_retries})")
...
def get_column_letter(n):
    """Converts a 0-indexed column number to a Google Sheets column letter (e.g., 0 -> A, 25 -> Z, 26 -> AA)."""
    result = ""
    n += 1
    while n > 0:
...
def process_single_image(file_info, headers, h_map, deadline_time, skip_sheet_row=False):
    ...
    try:
        ...
        return {"status": "success", "file_name": file_name, "serial": serial, "row": new_row, "file_id": file_id, "mapping": mapping}
...
def process_images():
    ...
    use_postgres = OCR_WRITE_TARGET == "postgres"
    ...
    logger.info(f"--- Batch Processing Started --- (write_target={OCR_WRITE_TARGET})")
    logger.info(f"Inbox Folder:   {mask_id(INBOX_FOLDER_ID)}")
    logger.info(f"Processed Fldr: {mask_id(PROCESSED_FOLDER_ID)}")

    if use_postgres:
        logger.info("PostgreSQL write: ocr_staging")
        if not DATABASE_URL_PG:
            return "Error: OCR_WRITE_TARGET=postgres requires DATABASE_URL."
        try:
            import psycopg2
            from postgres_ocr import fetch_existing_stg_ids
            conn = psycopg2.connect(DATABASE_URL_PG)
            try:
                existing_stg_ids = fetch_existing_stg_ids(conn)
            finally:
                conn.close()
        except Exception as e:
            logger.error(f"PostgreSQL connection failed: {e}")
            return f"Error: Cannot connect to PostgreSQL ({e})."
    else:
        logger.info(f"Spreadsheet ID: {mask_id(SPREADSHEET_ID)}")
        ...
        if len(rows) > 1:
            for idx, row in enumerate(rows[1:], start=2):
                ...
                if stg_val:
                    existing_stg_ids.add(stg_val.strip())
                ...
                if status == "登録待ち" and serial:
                    try:
                        qty_int = int(qty_val) if qty_val else 1
                    except Exception:
                        qty_int = 1
    ...
    for file in files:
        ...
        if file_stg_id in existing_stg_ids:
            logger.info(f"File {file['name']} already exists in Sheets (stg_id: {file_stg_id}). Skipping processing and moving to processed.")
            try:
                drive.files().update(
                    fileId=file['id'], 
                ...
                logger.error(f"Failed to move idempotent file {file['name']}: {e}")
            continue
    ...
    with ThreadPoolExecutor(max_workers=CONCURRENT_WORKERS) as executor:
        future_to_file = {
            executor.submit(process_single_image, file, headers, h_map, deadline_time, use_postgres): file
    ...
    logger.info(f"Processing {len(results)} individual results...")
    new_rows_dict = {}       
    updates_to_existing = {} 
    successful_file_ids = []
    ...
    if use_postgres:
        if success_count > 0:
            import psycopg2
            from postgres_ocr import insert_ocr_rows, mapping_to_row
            batch = []
            for r in results:
                if r.get("status") == "success" and r.get("mapping"):
                    batch.append(mapping_to_row(r["file_id"], r["file_name"], r["mapping"]))
            try:
                conn = psycopg2.connect(DATABASE_URL_PG)
                try:
                    insert_ocr_rows(conn, batch)
                finally:
                    conn.close()
            except Exception as e:
                logger.error(f"CRITICAL: PostgreSQL ocr_staging insert failed: {e}")
                return f"Error: PostgreSQL insert failed ({e}). Files remain in inbox for retry."
        sheets_ms = (time.time() - step_start) * 1000
    else:
        try:
            if success_count > 0:
                qty_updates = []
                col_letter_qty = get_column_letter(h_map['qty'])
                ...
                if qty_updates:
                    logger.info(f"Updating {len(qty_updates)} existing rows in sheet {mask_id(SPREADSHEET_ID)}...")
                ...
                if new_rows_dict:
                    logger.info(f"Appending {len(new_rows_dict)} new rows to sheet {mask_id(SPREADSHEET_ID)}...")
            ...
            logger.error(f"CRITICAL: Failed to update Google Sheets: {e}")
            return f"Error: Spreadsheet update failed ({e}). Files remain in inbox for retry."
...
def index():
    if not ENV_VALID:
        return "Service unavailable: Startup validation failed. Check logs.", 503
...
@app.route("/internal/ocr-job", methods=["POST"])
def process_ocr_job():
    if WORKER_SHARED_SECRET:
        token = request.headers.get("X-OCR-Secret", "")
    ...
    if not job_id:
        return {"ok": False, "message": "job_id required"}, 400
    if not DATABASE_URL_PG:
        return {"ok": False, "message": "DATABASE_URL required"}, 500
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """SELECT job_id, gcs_bucket, gcs_object_path, batch_id
                     FROM ocr_jobs
                    WHERE job_id = %s
                    LIMIT 1
                """,
            ...
            _, gcs_bucket, gcs_object_path, batch_id = row

        storage_client = storage.Client()
        blob = storage_client.bucket(gcs_bucket).blob(gcs_object_path)
        content = blob.download_as_bytes()
        content, _, _ = resize_image(content, max_size=1024)
        data, used_model, _ = call_gemini_with_hedging(content, "image/jpeg", str(job_id)[:8])
        if not data:
            raise Exception("OCR model failed")
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                """SELECT card_id
                     FROM cards
                    WHERE card_id = %s
                       OR (set_code = %s AND card_number_text = %s)
                    ORDER BY updated_at DESC
                    LIMIT 1""",
                (serial, set_code, card_num_str),
            )
            m = cur.fetchone()
            if m:
                duplicate_card_id = m[0]

            duplicate_status = "CANDIDATE" if duplicate_card_id else "NONE"
            merge_decision = "MERGE_EXISTING" if duplicate_card_id else "CREATE_NEW"
            image_url = f"https://storage.googleapis.com/{gcs_bucket}/{gcs_object_path}"
            stg_id = f"stg_job_{job_id}"
            cur.execute(
                """UPDATE ocr_staging
                      SET status='登録待ち',
                ...
                (
                    ...
                    data.get("number_total"),
                    data.get("rarity"),
                    data.get("card_type"),
                    data.get("trainer_subtype"),
                    data.get("poke_type"),
                    data.get("name_ja"),
        ...
        conn.commit()
        return {"ok": True, "job_id": job_id, "model": used_model}, 200
    except Exception as e:
        conn.rollback()
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE ocr_jobs SET status='FAILED', last_error=%s, updated_at=now() WHERE job_id=%s",
                (str(e), job_id),
            )
            cur.execute(
                "UPDATE ocr_staging SET status='OCR失敗', ocr_status='FAILED', last_error=%s, updated_at=now() WHERE stg_id=%s",
                (str(e), f"stg_job_{job_id}"),
            )
        conn.commit()
        logger.error(f"job failed job_id={job_id}: {e}")
        return {"ok": False, "message": str(e)}, 500
    finally:
        conn.close()

def validate_environment():
    """Validate environment variables and connectivity (Sheets or PostgreSQL)."""
    base_required = [
    ...
    if OCR_WRITE_TARGET == "postgres":
        required = base_required + [('DATABASE_URL', DATABASE_URL_PG)]
        ...
        try:
            ...
            conn = psycopg2.connect(DATABASE_URL_PG)
            conn.close()
            logger.info("Startup validation successful: PostgreSQL is reachable.")
        except Exception as e:
            logger.error(f"CRITICAL: Cannot connect to PostgreSQL: {e}")
            return False
        return True

    required = base_required + [('SPREADSHEET_ID', SPREADSHEET_ID)]
    missing = [name for name, val in required if not val]
    if missing:
        logger.error(f"CRITICAL: Missing environment variables: {', '.join(missing)}")
        return False
    ...
    try:
        ...
        logger.info(f"Startup validation successful: Spreadsheet {mask_id(SPREADSHEET_ID)} is accessible.")
    except Exception as e:
        logger.error(f"CRITICAL: Startup validation failed: Cannot access Spreadsheet {mask_id(SPREADSHEET_ID)}. Error: {e}")
        return False
    return True
...
if __name__ == "__main__":
    if not ENV_VALID:
        logger.error("System startup failed due to environment validation errors.")
