...
// Google Apps Scriptの実行制限（6分）を考慮した最大実行時間 (ミリ秒)
// 5分 (300,000ms) を上限とする
...
function processInboxImages() {
  ...
  try {
    ...
    Logger.log(`LockServiceエラー: ${e.message}`);
    return;
  }
  ...
  try {
    ...
    while (filesIterator.hasNext()) {
      ...
      if (processedCount >= MAX_FILES_PER_RUN) {
        Logger.log(`最大処理件数 (${MAX_FILES_PER_RUN}) に達したため終了します。`);
        break;
      }
      ...
      if (elapsedTime > MAX_EXECUTION_TIME_MS) {
        Logger.log(`実行時間制限 (${MAX_EXECUTION_TIME_MS / 1000}秒) に近づいたため、安全に打ち切ります。経過時間: ${elapsedTime / 1000}秒`);
        break;
      }
